...
import { getProductById, getProductRecommendations, logProductView, addProductToCart } from '../api';
  ...
  useEffect(() => {
    const fetchProductAndRecommendations = async () => {
      try {
        ...
        if (DEMO_USER_ID) {
          logProductView(DEMO_USER_ID, productId).catch(err => console.error("Failed to log view:", err));
        }
        ...
        setError('Failed to load product details or recommendations. ' + err.message);
        console.error(err);
      } finally {
        setLoading(false);
      }
    ...
    <div className="product-detail-page">
      <div className="product-detail-card">
        ...
        <div className="product-detail-info">
          ...
          <p className="product-detail-stock">In Stock: {product.stock_quantity}</p>
                ...
                {Object.entries(parsedAttributes).map(([key, value]) => (
                  <li key={key}>
                    ...
                    {Array.isArray(value)
                      ...
                      ? JSON.stringify(value) // Convert objects to string representation
                      : value}
                  </li>
                ))}
              </ul>
      ...
      <section className="recommendations-section">
        ...
        {recommendations.length > 0 ? (
          ...
          <p className="no-recommendations">No specific recommendations found. Explore other products!</p>
        )}
      </section>
    </div>
  );
